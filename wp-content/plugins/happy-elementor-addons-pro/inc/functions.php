...
use Happy_Addons_Pro\Classes\Mega_Menu;
 ...
 * @return string
 */
function hapro_get_initial_load_cookie_value($cookie_name,$cookie_value,$expire_time,$cookiearray,$to) {
	//print_r($cookie_name);
	$datacookie=$cookie_name."_data";
	if(isset($_COOKIE[$datacookie]) && !empty($_COOKIE[$datacookie]) ){
		$cval= stripslashes($_COOKIE[$datacookie]);
	...
	if(isset($_COOKIE[$cookie_name]) && !empty($_COOKIE[$cookie_name]) ){
		...
		$dateex= gmdate("D, d M Y H:i:s", $expire_time) . " UTC";
		$issecure = stripos($_SERVER['SERVER_PROTOCOL'],'https') === 0 ? 'secure' : '';
		$js_code = "document.cookie = '". $datacookie ." = ".$cookiearray." ;expires=" .$dateex. ";".$issecure."';";
		$js_code .= "document.cookie = '". $cookie_name ." = ".$cookie_value." ;expires=" .$dateex. ";".$issecure."';";
		$js_code = "document.addEventListener('DOMContentLoaded', function () {".$js_code."});";
		wp_add_inline_script( 'happy-addons-pro', $js_code, 'after' );
		return true;
	}
}
